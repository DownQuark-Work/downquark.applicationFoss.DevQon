...
// fn main() {
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      build::handle_tauri_setup(app);
      state::get_all_states(app); // TODO: method name may need to change?
      Ok(())
    })
    .on_window_event(|event| { utils::helpers::build::run_frontend_in_background(event) })
    .manage(commands::_devqon::Database::default()) // TODO: update or remove - this is temporary for now
