...
impl DevQonConfig {
  pub fn make_config(sys_conf_paths: HashMap<&str, String>) -> Result<Self, ConfigError> {
    ...
    let devqon_conf = Config::builder() // as sources are added below duplicate keys will overwrite previously defined values
        ...
        .set_override("devqon.directory_parsed.home", "TEMPORARY::sys_conf_paths[HOME]--UNCOMMENT_BELOW_WHEN_READY".to_string())? // programmatically change settings after parsing the `sys_conf_paths`
        .set_override("devqon.directory_parsed.application", "TEMPORARY::sys_conf_paths[APPLICATION]--UNCOMMENT_BELOW_WHEN_READY".to_string())?
        .set_override("devqon.directory_parsed.resource", "TEMPORARY::sys_conf_paths[RESOURCE]--UNCOMMENT_BELOW_WHEN_READY".to_string())?
